...
def server():
    try:
        ...
        def res(client):
            ...
            if shell == 1 and "cwd" in r:
                client.send(os.getcwd())
        #    if shell == 1 and "file=upload text=" in r:
        #        file=open("file_send.ini", "w")
        #        file.write(r[19:])
        #        file.close()
        #        config.read("file_send.ini")
        #        name = config["FILE"]["name"]
        #        txt = config["FILE"]["txt"]
        #        file=open(name, "w")
        #        file.write(txt)
        #        file.close()
        #        subprocess.call("rm file_send.ini", shell=True)
            if shell == 1 and "ls" in r:
                r = r + ' ' + os.getcwd()
            if shell == 1:
                client.send(l + '\033[0m.....................')
